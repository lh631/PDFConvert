...
namespace Controls
{
    public partial class ListViewPlus : ListView
    {
        ...
        private Brush m_StatusPersentValueBrush = new SolidBrush(Color.FromArgb(41, 174, 255));
        ...
        {
            imgs = new ImageList();
            imgs.ImageSize = new System.Drawing.Size(24, 24);
        ...
        private void InitColumns()
        {
            ...
            this.Columns.AddRange(new ColumnHeader[] { recNo, fileName, select, status, operate });
        }
            ...
            try
            {
                ...
                switch (e.ColumnIndex)
                {
                    ...
                    case COL_FILENAME:
                        ...
                        if (img == null) return;
                        e.Graphics.DrawImage(img, e.Bounds.Left + 5, e.Bounds.Top + 10, 25, 25);
                        Rectangle r = e.Bounds;
                        r.X += 40;
                        r.Width -= 40;
                        TextRenderer.DrawText(e.Graphics,
                    ...
                    case COL_STATUS:
                        //进度条
                        e.Graphics.FillRectangle(m_StatusBrush, e.Bounds.X + 10, e.Bounds.Y + 20, e.Bounds.Width - 55, 8);
                        e.Graphics.FillRectangle(m_StatusPersentValueBrush, e.Bounds.X + 10, e.Bounds.Y + 20, (e.Bounds.Width - 55) * (info.PersentValue / 100f), 8);
                        //按钮
                        if (e.ItemState == ListViewItemStates.Grayed) return;
                        Image imgstatus = GetStatusImage(info.Status);
                        if (imgstatus == null) return;
                        e.Graphics.DrawImage(imgstatus, e.Bounds.X + e.Bounds.Width - STATUSBUTTONXPOS, e.Bounds.Top + STATUSBUTTONYPOS, 20, 20);
                        if (info.StatusChanged)
                        {
                            info.StatusChanged = false;
                            this.Invalidate(new Rectangle(e.Bounds.X + e.Bounds.Width - STATUSBUTTONXPOS, e.Bounds.Top + STATUSBUTTONYPOS, 20, 20));
                        }
                        break;
                    //操作按钮
                    case COL_OPERATE:
                        e.Graphics.DrawString(OpenButtonText, ItemFont, m_OpenTextBrush, new Rectangle(e.Bounds.X + e.Bounds.Width / 2 - 50, e.Bounds.Top + 15, 40, 18));
                        ...
                        e.Graphics.DrawImage(imgopen, e.Bounds.X + e.Bounds.Width / 2 - 5, e.Bounds.Top + 12);
                        ...
                        e.Graphics.DrawImage(imgdelete, e.Bounds.X + e.Bounds.Width / 2 + 35, e.Bounds.Top + 12, imgdelete.Width, imgdelete.Height);
